...
function lessonlms_courses_target_audience_meta_box() {
    add_meta_box(
        'courses_target_audience',
        'Who This Course is For',
        'lessonlms_courses_target_audience_callback',
        'courses',
        'side', // sidebar
        'high'
    );
}
add_action('add_meta_boxes', 'lessonlms_courses_target_audience_meta_box');
function lessonlms_courses_target_audience_callback($post) {
    $audience = get_post_meta($post->ID, 'course_target_audience', true);
    ...
    <div id="audience-wrapper">
        ...
        <div class="d-flex mb-2">
            <input type="text" id="new-audience-input" class="form-control" placeholder="Enter new item" style="margin-right:5px;" />
            <button type="button" id="add-audience" class="button button-primary">Add</button>
        </div>

        <ul id="audience-list" style="list-style:none; margin:0; padding:0;">
            <?php foreach($audience as $index => $item): ?>
                <li class="audience-item d-flex align-items-center justify-content-between mb-1" style="cursor:move;" data-index="<?php echo $index; ?>">
                    <span class="audience-text"><?php echo esc_html($item); ?></span>
                    <div>
                        <button type="button" class="edit-audience btn btn-sm btn-secondary">Edit</button>
                        <button type="button" class="remove-audience btn btn-sm btn-danger">&times;</button>
                    </div>
                    <input type="hidden" name="course_target_audience[]" value="<?php echo esc_attr($item); ?>" />
                </li>
            <?php endforeach; ?>
        </ul>
    </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <input type="text" id="audience-modal-input" class="form-control" />
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" id="update-audience">Update</button>
          </div>
        </div>
      </div>
    </div>
...
jQuery(document).ready(function($){
    ...
    const audienceModal = new bootstrap.Modal(modalEl);

    // sortable
    $('#audience-list').sortable({
        placeholder: "ui-state-highlight"
    });

    // ADD
    $('#add-audience').on('click', function(){
        let val = $('#new-audience-input').val().trim();
        if(val === '') return;

        let newItem = $(`
            <li class="audience-item d-flex align-items-center justify-content-between mb-1" style="cursor:move;">
                <span class="audience-text"></span>
                <div>
                    <button type="button" class="edit-audience btn btn-sm btn-secondary">Edit</button>
                    <button type="button" class="remove-audience btn btn-sm btn-danger">&times;</button>
                </div>
                <input type="hidden" name="course_target_audience[]" value="">
            </li>
        `);

        newItem.find('.audience-text').text(val);
        newItem.find('input').val(val);

        $('#audience-list').append(newItem);
        $('#new-audience-input').val('');
    });

    // REMOVE
    $('#audience-list').on('click', '.remove-audience', function(){
        $(this).closest('.audience-item').remove();
    });

    // EDIT (FIXED)
    $('#audience-list').on('click', '.edit-audience', function(){
        currentItem = $(this).closest('.audience-item'); // ✅ item store
        let text = currentItem.find('.audience-text').text();
        $('#audience-modal-input').val(text);
        audienceModal.show(); // ✅ single instance
    });

    // UPDATE (FIXED)
    $('#update-audience').on('click', function(){
        if(!currentItem) return;

        let newVal = $('#audience-modal-input').val().trim();
        if(newVal === '') return;

        currentItem.find('.audience-text').text(newVal);
        currentItem.find('input[type="hidden"]').val(newVal);
    ...
    <style>
    .ui-state-highlight { height:30px; background:#f0f0f0; border:1px dashed #ccc; margin-bottom:5px; }
    </style>
    <?php
}
...
function lessonlms_courses_save_target_audience_meta($post_id) {
    if(!isset($_POST['course_target_audience_nonce']) || !wp_verify_nonce($_POST['course_target_audience_nonce'], 'save_course_target_audience')) return;

    if(isset($_POST['course_target_audience']) && is_array($_POST['course_target_audience'])){
        $audience = array_filter(array_map('sanitize_text_field', $_POST['course_target_audience']));
        update_post_meta($post_id, 'course_target_audience', $audience);
    } else {
        delete_post_meta($post_id, 'course_target_audience');
    }
}
add_action('save_post_courses', 'lessonlms_courses_save_target_audience_meta');
