...
function lessonlms_handle_enrollmemt()
{
    ...
    $cource = $_POST['course_id'];
    $course_id = isset($cource) ? intval($cource) : 0;
...
add_action('wp_ajax_noprev_enroll_course', 'lessonlms_handle_enrollmemt');
...
function lessonlms_dashboard_enrollment_widget(){
  ...
  $total_enrollments = $wpdb-> get_var(
    ...
    $recent_enrollments = $wpdb->get_results(
        ...
        "SELECT u.user_login, p.post_title, un.meta_value
