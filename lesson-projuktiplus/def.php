...
function amars_single_course_template($template) {
    if (is_singular('amars_course')) {
        $new_template = locate_template(array('single-amars_course.php'));
        if ('' != $new_template) {
            return $new_template;
        }
        
        // Default template
        add_filter('the_content', 'amars_display_course_content');
    }
    return $template;
}
add_filter('template_include', 'amars_single_course_template');
...
function amars_single_lesson_template($template) {
    if (is_singular('amars_lesson')) {
        $new_template = locate_template(array('single-amars_lesson.php'));
        if ('' != $new_template) {
            return $new_template;
        }
        
        add_filter('the_content', 'amars_display_lesson_content');
    }
    return $template;
}
add_filter('template_include', 'amars_single_lesson_template');

function amars_display_lesson_content($content) {
    if (is_singular('amars_lesson')) {
        global $post;
        $lesson_id = $post->ID;
        ...
        ob_start();
        ?>
        <div class="amars-lesson-content">
            <div class="lesson-meta">
                <?php if ($course): ?>
                    <div class="meta-item">
                ...
                <?php if ($module): ?>
                    <div class="meta-item">
                        <strong>Module:</strong> <?php echo esc_html($module->name); ?>
                    </div>
                <?php endif; ?>
            ...
            <?php if ($video): ?>
                <div class="lesson-video">
                    ...
                    if (strpos($video, 'youtube.com') !== false || strpos($video, 'youtu.be') !== false) {
                        ...
                        if (isset($matches[1])) {
                            echo '<iframe width="560" height="315" src="https://www.youtube.com/embed/' . $matches[1] . '" frameborder="0" allowfullscreen></iframe>';
                        }
                    } else {
                        echo '<p><a href="' . esc_url($video) . '" target="_blank">Watch Video</a></p>';
                    }
                    ?>
                </div>
            <?php endif; ?>
            ...
            <div class="lesson-navigation">
                ...
                if ($current_index !== false) {
                    ...
                    if ($current_index > 0) {
                        ...
                        echo '<a href="' . get_permalink($prev_lesson->ID) . '" class="prev-lesson">← Previous Lesson</a>';
                    }
                    
                    // Back to course
                    if ($course) {
                        echo '<a href="' . get_permalink($course_id) . '" class="back-to-course">Back to Course</a>';
                    }
                    
                    // Next lesson
                    if ($current_index < count($all_lessons) - 1) {
                        $next_lesson = $all_lessons[$current_index + 1];
                        echo '<a href="' . get_permalink($next_lesson->ID) . '" class="next-lesson">Next Lesson →</a>';
                    }
...
function amars_courses_list_shortcode($atts) {
    ...
    if ($atts['instructor']) {
        $user = get_user_by('login', $atts['instructor']);
        if ($user) {
            $args['author'] = $user->ID;
        }
    }
    ...
    if ($courses->have_posts()):
        echo '<div class="amars-courses-list">';
        while ($courses->have_posts()):
            $courses->the_post();
            $course_id = get_the_ID();
            $modules_count = count(get_terms(array(
                'taxonomy' => 'amars_module',
                'hide_empty' => false,
                'meta_key' => '_module_course_id',
                'meta_value' => $course_id,
            ...
            <div class="course-card">
                ...
                <div class="course-content">
                    ...
                    <div class="course-meta">
                        <span class="modules-count"><?php echo $modules_count; ?> Modules</span>
                        <a href="<?php the_permalink(); ?>" class="course-link">View Course</a>
                    </div>
                </div>
            </div>
            <?php
...
add_shortcode('amars_courses', 'amars_courses_list_shortcode');

// ============================================
// 11. ADMIN FILTERS & QUERIES
// ============================================

// Filter courses by instructor
function amars_filter_courses_by_instructor($query) {
    if (!is_admin() || !$query->is_main_query()) return;
    
    $screen = get_current_screen();
    if (!$screen || $screen->post_type != 'amars_course') return;
...
add_action('pre_get_posts', 'amars_filter_courses_by_instructor');

// Filter modules by instructor
function amars_filter_modules_by_instructor($args, $taxonomies) {
    if (!is_admin()) return $args;
    
    if (in_array('amars_module', $taxonomies)) {
        $user = wp_get_current_user();
        if (in_array('amars_instructor', (array) $user->roles)) {
            $my_courses = get_posts(array(
                'post_type' => 'amars_course',
            ...
            if (!empty($my_courses)) {
                ...
                $args['meta_key'] = '_module_course_id';
                $args['meta_value'] = '0';
            }
        }
    }
...
add_filter('get_terms_args', 'amars_filter_modules_by_instructor', 10, 2);

// Filter lessons by instructor
function amars_filter_lessons_by_instructor($query) {
    if (!is_admin() || !$query->is_main_query()) return;
    
    $screen = get_current_screen();
    if (!$screen || $screen->post_type != 'amars_lesson') return;
...
add_action('pre_get_posts', 'amars_filter_lessons_by_instructor');

// ============================================
// 12. MODULE CONSISTENCY CHECKER
// ============================================

function amars_module_consistency_checker() {
    ?>
    <div class="wrap">
        <h1>Module Consistency Checker</h1>
        ...
        foreach ($modules as $module) {
            ...
            // Check module has course
            if (!$course_id) {
                $issues[] = "Module '{$module->name}' has no course assignment.";
                continue;
            }
            ...
            $course = get_post($course_id);
            if (!$course) {
                $issues[] = "Module '{$module->name}' is assigned to non-existent course.";
            }
        }
        
        if (empty($issues)) {
            echo '<div class="notice notice-success"><p>✅ All modules are properly assigned.</p></div>';
        } else {
            echo '<div class="notice notice-warning"><p><strong>Found ' . count($issues) . ' issues:</strong></p><ul>';
        ...
        <form method="post" action="">
            <?php wp_nonce_field('amars_fix_issues', 'amars_fix_nonce'); ?>
            <p>
                <input type="submit" name="fix_issues" class="button button-primary" value="Fix Issues">
                <span class="description">This will reassign modules to correct courses.</span>
            </p>
        </form>
        
        <?php
        if (isset($_POST['fix_issues']) && check_admin_referer('amars_fix_issues', 'amars_fix_nonce')) {
            $fixed = 0;
            foreach ($modules as $module) {
                $course_id = get_term_meta($module->term_id, '_module_course_id', true);
                if ($course_id && !get_post($course_id)) {
                    delete_term_meta($module->term_id, '_module_course_id');
                    $fixed++;
                }
            }
            echo '<div class="notice notice-success"><p>✅ Fixed ' . $fixed . ' module assignments.</p></div>';
        }
        ?>
    </div>
    <?php
