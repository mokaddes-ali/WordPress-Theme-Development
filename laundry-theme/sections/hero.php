...
/** * Hero Section Template * * @package LaundryTheme */ ?> <section class="relative w-full">
    <div class="hero-slick-items">
        <?php $slider_args = array('post_type' => 'slider', 'posts_per_page' => 3, 'orderby' => 'date', 'order' => 'DESC');
        $slider_posts = get_posts($slider_args);
        foreach ($slider_posts as $post): setup_postdata($post); ?>
            <?php $slider_background_image = has_post_thumbnail($post) ? get_the_post_thumbnail_url($post, 'full') : get_template_directory_uri() . '/assets/images/Image_here.png';
            ...
            <div class="hero-slick-single-item relative h-[385px] sm:h-[350px] md:h-[420px] lg:h-[500px]">
                <div> 
                    <?php if ($slider_background_image) : ?> <img
                            src="<?php echo esc_url($slider_background_image); ?>"
                            alt="<?php echo esc_attr($slider_title); ?>"
                            class="absolute inset-0 z-0 w-full h-full object-cover" /> <?php endif; ?> </div> <!-- overlay -->
                <div class="absolute inset-0 z-10 bg-gradient-to-r from-[#142137] to-[#1421371A] opacity-100"> </div>
                <!-- Hero Content -->
                <div
                    class="z-50 relative flex flex-col gap-3 px-[5%] lg:px-[5%] 2xl:px-[8%] py-4 sm:py-6 md:py-6 lg:py-13 mx-auto text-white text-center md:text-start">
                    <!-- Tag Box -->
                    <div class="flex items-center justify-center md:justify-start">
                        <div
                            class="flex items-center gap-[0.5rem] px-[0.50rem] py-[0.3rem] bg-white/15 backdrop-blur-[0.4375rem] text-white/70 text-sm md:text-base font-poppins font-normal md:font-medium">
                            <div class="w-[0.3125rem] h-[0.3125rem] rounded-full bg-white flex-shrink-0"> </div>
                            <?php if ($slider_type): ?> <?php echo esc_html($slider_type); ?> <?php endif; ?>
                        </div>
                    </div> <!-- Heading -->
                    <h1
                        class="text-white font-poppins w-full text-[1.50rem] sm:text-[2.75rem] md:text-[3.0rem] lg:text-[4.50rem] md:max-w-2xl lg:max-w-4xl xl:max-w-5xl font-semibold leading-[1.1] tracking-[-0.125rem]">
                        <?php if ($slider_title): ?> <?php echo esc_html(wp_trim_words($slider_title, 6)); ?> <?php endif; ?>
                    </h1> <!-- Description -->
                    <p
                        class="text-white/70 font-poppins w-full md:w-[80%] lg:w-[48.125rem] md:pt-[1.5rem] text-sm sm:text-base md:text-[1.115rem] font-medium leading-[1.8]">
                        <?php if ($slider_description): ?> <?php echo esc_html(wp_trim_words($slider_description, 25),); ?>
                        <?php endif; ?> </p> <!-- Button & Avatar Section -->
                    <div
                        class="flex justify-center flex-col sm:justify-start sm:flex-row items-center sm:items-center pt-1 md:pt-[1.5rem] gap-[1.25rem] sm:gap-[2rem]">
                        <button
                            class="px-3 py-2 lg:px-5 lg:py-4 flex-shrink-0 bg-[#4375E7] text-white font-[Poppins] text-[12px] lg:text-[0.9375rem] font-medium hover:bg-blue-600 transition">
                            <?php if ($slider_button_text): ?> <a
                                    href="<?php echo esc_url($slider_button_link); ?>"><?php echo esc_html($slider_button_text); ?></a>
                            <?php endif; ?> </button>
                        <div class="flex items-center gap-2 md:gap-[1.125rem]"> <img
                                src="<?php echo esc_url($slider_avatar); ?>" alt="Client"
                                class="wimg-[10.125rem] h-[3.375rem]" />
                            <div class="flex flex-col gap-[0.5rem] sm:gap-[0.625rem]">
                                <div class="flex items-center text-yellow-400"> <?php if ($slider_rating): ?>
                                        <?php $total_stars = $slider_rating;
                                                                                                  for ($i = 1; $i <= $total_stars; $i++): ?>
                                                                                                   <svg
                                                xmlns="http://www.w3.org/2000/svg" width="1rem" height="1rem" viewBox="0 0 20 20"
                                            ...
                                            </svg> <?php endfor; ?> <?php endif; ?> </div>
                                <p class="text-white font-poppins text-[12px] md:text-[1rem] font-medium leading-[1.5]">
                                    <?php if ($slider_rating_text): ?> <?php echo esc_html($slider_rating_text); ?>
                                    <?php endif; ?> </p>
                            </div>
                        </div>
                    </div>
    ...
    <div class="absolute top-[64%] left-[85%] z-30 flex flex-col gap-2">
        <!-- Prev Button --> <button id="custom-prev"
            class="w-10 h-10 lg:w-[54px] lg:h-[54px] flex items-center justify-center rounded-full hover:bg-[#4375E7] transition duration-300">
            ...
            </svg> </button> <!-- Next Button --> <button id="custom-next"
            class="w-10 h-10 lg:w-[54px] lg:h-[54px] flex items-center justify-center rounded-full hover:bg-[#4375E7] transition duration-300">
            <svg xmlns="http://www.w3.org/2000/svg" width="54" height="54" viewBox="0 0 54 54" fill="none">
